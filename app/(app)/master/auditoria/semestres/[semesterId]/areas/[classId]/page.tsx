...
import { getMasterClosedSemesterAreaDetail } from "@/services/master";
import { getAuthenticatedClassFinalReport } from "@/services/reports";

export default async function MasterClosedSemesterAreaAuditPage(props: {
  params: Promise<{
    semesterId: string;
    classId: string;
  }>;
  ...
  const [areaDetail, classReportResult] = await Promise.all([
    getMasterClosedSemesterAreaDetail(semesterId, classId),
    ...
    return (
      <div className="stack audit-page">
        <section className="hero-card">
          <p className="eyebrow">Auditoria global por área</p>
          <h1>Área encerrada não encontrada</h1>
          <p>
            Não foi possível localizar esta área arquivada dentro do semestre
            selecionado na auditoria global.
          </p>
          <div className="actions-row">
            <a
              href={
                semesterId
                  ? `/master/auditoria?semestre=${encodeURIComponent(semesterId)}`
                  : "/master/auditoria"
              }
              className="button button-secondary"
            >
              Voltar para auditoria global
            </a>
          </div>
        </section>
      </div>
  ...
  return (
    <div className="stack audit-page audit-area-page">
      <section className="hero-card">
        <p className="eyebrow">Auditoria global por área encerrada</p>
        ...
        <div className="actions-row">
          <a
            href={`/master/auditoria?semestre=${encodeURIComponent(areaDetail.semester.id)}`}
            className="button button-secondary"
          >
            Voltar ao semestre encerrado
          </a>
          <a
            href={`/relatorios/turmas/${areaDetail.area.classId}?from=master-audit&semestre=${encodeURIComponent(areaDetail.semester.id)}&print=1`}
            className="button"
          >
            Imprimir relatório final
          </a>
          <a
            href={`/relatorios/export/turmas/${areaDetail.area.classId}/excel?semestre=${encodeURIComponent(areaDetail.semester.id)}&from=master-audit`}
            className="button button-secondary"
          >
            Exportar Excel
          </a>
      ...
      <SectionCard
        ...
        description="Dados principais da área arquivada e do encerramento do semestre na visão global do Master."
      >
              ...
              <tbody>
                {report.students.map((student) => (
                  <tr key={student.enrollmentId}>
                    ...
                    <td>
                      <a
                        href={`/master/auditoria/semestres/${encodeURIComponent(areaDetail.semester.id)}/areas/${encodeURIComponent(areaDetail.area.classId)}/alunos/${encodeURIComponent(student.studentId)}?matricula=${encodeURIComponent(student.enrollmentId)}`}
