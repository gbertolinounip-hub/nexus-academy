...
import { reviewClinicalTreatmentPlanAction } from "@/app/(app)/clinica-supervisionada/[caseId]/plano-tratamento/actions";
import {
  initialClinicalTreatmentPlanReviewActionState,
  type ClinicalTreatmentPlanReviewFormValues
...
interface ClinicalTreatmentPlanReviewFormProps {
  caseId: string;
  recordId: string;
...
function buildResolvedValues(
  ...
  initialStatus: ClinicalRecordStatus,
  initialFeedback?: string | null,
  submittedValues?: ClinicalTreatmentPlanReviewFormValues
) {
  return {
...
export function ClinicalTreatmentPlanReviewForm({
  caseId,
  recordId,
  initialStatus,
  initialFeedback
}: ClinicalTreatmentPlanReviewFormProps) {
  const router = useRouter();
  const [state, formAction] = useActionState(
    reviewClinicalTreatmentPlanAction,
    initialClinicalTreatmentPlanReviewActionState
  );
  const safeState = state ?? initialClinicalTreatmentPlanReviewActionState;
    ...
    <form action={formAction} className="form-stack clinical-evaluation-review-form">
      ...
      >
        ...
        <textarea
          ...
          placeholder="Registre orientações, aprovação ou ajustes solicitados para este plano de tratamento."
        />
